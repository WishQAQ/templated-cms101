...
EOT;
    ...
    while($dbrow=$dsql->GetObject())
    {
        ...
        {
            $link = "<p><a href='".$dbrow->url."' target='_blank'>".cn_substr($dbrow->webname,$titlelen)."</a></p> ";
        }
        else if($type=='image')
        {
            $link = "<p><a href='".$dbrow->url."' target='_blank'><img src='".$dbrow->logo."' width='88' height='31' border='0'></a></p> ";
        }
        else
        {
            if($dbrow->logo=='')
